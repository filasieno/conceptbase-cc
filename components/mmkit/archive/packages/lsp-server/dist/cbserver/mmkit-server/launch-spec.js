...
exports.executableLaunchEnv = executableLaunchEnv;
exports.dockerContainerEnv = dockerContainerEnv;
exports.buildLaunchSpec = buildLaunchSpec;
const path = __importStar(require("node:path"));
const docker_defaults_1 = require("./docker-defaults");
const executable_path_1 = require("./executable-path");
function setIfNonEmpty(env, key, value) {
...
function isNativeBundledExecutable(snapshot) {
    const s = snapshot.server;
    if (s.devCommand)
        return true;
    if (process.env.MMKIT_CBSERVER_BIN)
        return true;
    const cmd = (0, executable_path_1.resolveExecutableCommand)(snapshot.paths, s.executablePath, s.devCommand);
    return cmd.includes("/nix/store/") || path.isAbsolute(cmd);
}
function executableLaunchEnv(snapshot) {
    const s = snapshot.server;
    ...
    if (!native) {
        env.CB_HOME = docker_defaults_1.DOCKER_IMAGE_RUNTIME.cbHome;
        env.CB_POOL = docker_defaults_1.DOCKER_IMAGE_RUNTIME.cbPool;
        env.CBS_DIR = docker_defaults_1.DOCKER_IMAGE_RUNTIME.cbsDir;
        env.CBL_DIR = docker_defaults_1.DOCKER_IMAGE_RUNTIME.cblDir;
        env.PROLOG_VARIANT = docker_defaults_1.DOCKER_IMAGE_RUNTIME.prologVariant;
        setIfNonEmpty(env, "CB_VARIANT", docker_defaults_1.DOCKER_IMAGE_RUNTIME.cbVariant);
    }
...
function dockerContainerEnv(snapshot) {
    const s = snapshot.server;
    const env = {
        CB_PORTNR: String(s.port),
        CB_UPDATE_MODE: s.updateMode,
        TMPDIR: docker_defaults_1.DOCKER_IMAGE_RUNTIME.containerTmp,
        CB_DB_ALL: docker_defaults_1.DOCKER_IMAGE_RUNTIME.containerWorkspace,
...
function applyOptionalServerFlags(env, s) {
...
function buildLaunchSpec(snapshot) {
    const s = snapshot.server;
    const p = snapshot.paths;
    if (s.launchKind === "docker") {
        ...
        const args = [
            ...
            `${s.port}:${s.port}`,
            "-v",
            `${p.databaseAllPath}:${docker_defaults_1.DOCKER_IMAGE_RUNTIME.containerWorkspace}`,
            "-v",
            `${p.tmpDir}:${docker_defaults_1.DOCKER_IMAGE_RUNTIME.containerTmp}`,
            "-v",
    ...
    const command = (0, executable_path_1.resolveExecutableCommand)(p, s.executablePath, s.devCommand);
    const args = [...s.extraArgs];
    ...
    if (env.CB_LOAD_DIR)
        args.push("-load", env.CB_LOAD_DIR);
    if (env.CB_SAVE_DIR)
        args.push("-save", env.CB_SAVE_DIR);
    if (env.CB_VIEWS_DIR)
        args.push("-views", env.CB_VIEWS_DIR);
