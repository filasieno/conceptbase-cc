...
```bash
nix build .#checks.x86_64-linux.define-recursive-queries-on-cyclic-structures
```

== Input

=== `Barberism/barber3a.sml.txt`
...
Class Man  with
  ...
  rule
    r2: $ forall m1,m2/Man 
             (exists m/Man (m1 shaves m) and (m shaves_trans m2))
          ==> (m1 shaves_trans m2) $;
    r1: $ forall m1,m2/Man (m1 shaves m2) ==> (m1 shaves_trans m2) $
end
...
* Creation: 19-Mar-2002
* ----------------------------------------------------------------------
* The query will run with ConceptBase V5.2.3 or later.
* ConceptBase V5.2.2 returns only 'king' which is incomplete.
}
...
* File: Barber.sml
* Author: Manfred Jeusfeld
* Creation: 11-Mar-2002 (5-Oct-2005)
* ----------------------------------------------------------------------
* This example is about a non-stratified rule. 
* The ShavedByBarber query will return 'pete' but not 'TheBarber'.
* The query will run only with ConceptBase V5.2.2 or later.
* Starting with V6.1.3, ConceptBase can detect some stratification
* violations like this and reports warnings.
*
...
{* in Datalog:
   ...
   ShavedByBarber(X) :- man(X), shaves(thebarber,X).
...
{*
    NotShavedByBarber(X) :- man(X), not shaves(thebarber,X).
*}
...
{*
    DoesntShaveHimself(X) :- man(X), not shaves(X,X).
*}
...
{
*
* File: BigFlights2.sml
* Author: Manfred Jeusfeld
* Creation: 6-Feb-2002 (10-Dec-2003/M.Jeusfeld)
* ----------------------------------------------------------------------
* A version of BigFlights.sml that uses meta formulas to
* express transitive closure.
* The queries will run only with ConceptBase V6.1.1 or later.
}

{* Some generic meta formulas for transitive closure *}
...
attributee city!con_to in Class with 
   attribute
	departure: Integer;	
	arrival: Integer;
	flnr: String
...
{* This will trigger generation of formulas defining the transitive closure *}
city!trans_con_to with
...
{
*
* File: EvenOddBasic.sml
* Author: Manfred Jeusfeld
* Creation: 16-Noc-2002 (17-Nov-2005/M.Jeusfeld)
* ----------------------------------------------------------------------
* Variant of EvenOdd.sml which does not use Telos integers
* but simulates number by objects with successor links.
*
* Requires ConceptBase 6.2 released after 17-Nov-2005.
}
...
Individual Odd isA Int end  {* this needs to be defined in advance to 'Even' *}

QueryClass Even isA Int with
  constraint
    c: $ (this = ZERO) or (exists y/Odd (y succ ~this)) $
end

QueryClass Odd isA Int with
  constraint
    c: $ exists y/Even (y succ ~this)  $
end
...
* File: EvenOdd.sml
* Author: Manfred Jeusfeld
* Creation: 11-Sep-2002 (18-Dec-2008/M.Jeusfeld)
* ----------------------------------------------------------------------
* This is another the O-Telos representation of the 'even' example of LOLA
* at http://daisy.fmi.uni-passau.de/lola/
* Call the two queries Even and Odd to see whether they are working
* as expected. Note that some integers are pre-defined in ConceptBase.
* Hence, you might want to display the instances of Integer before
* call one of the two queries.
*
* Requires ConceptBase 7.1 released December 2008
}

{* We use Int instead Integer because the '+' function in the queries below *}
{* generates new instances of Integer on the fly. Thus, using Integer as    *}
{* range would force the queries Even and Odd into infinite loops.          *}
{* Int is limited to the range [0,99].                                     *}

Int in Class isA Integer with
  rule
    axiom1: $ (0 in Int) $;
    axiom2: $ forall n/Int n1/Integer (n1 = n+1) and (n < 100)
                 ==> (n1 in Int) $
end

Individual Odd end  {* this needs to be defined in advance to 'Even' *}
...
```text
=== HOW-TO: define-recursive-queries-on-cyclic-structures ===

>>> Telling ./Barber.sml.txt
This is CBShell, the command line interface to ConceptBase.cc
[offline]>Successfully connected to server
[localhost:4001]>yes
[localhost:4001]>
>>> Telling ./Barberism/barber3a.sml.txt
This is CBShell, the command line interface to ConceptBase.cc
[offline]>Successfully connected to server
[localhost:4001]>no
[localhost:4001]>
>>> Telling ./Barberism/barber3b.sml.txt
This is CBShell, the command line interface to ConceptBase.cc
[offline]>Successfully connected to server
[localhost:4001]>no
[localhost:4001]>
>>> Telling ./Barberism/barber3c.sml.txt
This is CBShell, the command line interface to ConceptBase.cc
[offline]>Successfully connected to server
[localhost:4001]>no
[localhost:4001]>
>>> Telling ./BigFlights.sml.txt
This is CBShell, the command line interface to ConceptBase.cc
[offline]>Successfully connected to server
[localhost:4001]>no
[localhost:4001]>
>>> Telling ./BigFlights2.sml.txt
This is CBShell, the command line interface to ConceptBase.cc
[offline]>Successfully connected to server
[localhost:4001]>no
[localhost:4001]>
>>> Telling ./EvenOdd.sml.txt
This is CBShell, the command line interface to ConceptBase.cc
[offline]>Successfully connected to server
[localhost:4001]>yes
[localhost:4001]>
>>> Telling ./EvenOddBasic.sml.txt
This is CBShell, the command line interface to ConceptBase.cc
[offline]>Successfully connected to server
[localhost:4001]>no
[localhost:4001]>
>>> Telling ./ObjectDep.sml.txt
This is CBShell, the command line interface to ConceptBase.cc
[offline]>Successfully connected to server
[localhost:4001]>yes
[localhost:4001]>
>>> Telling ./Russel.sml.txt
This is CBShell, the command line interface to ConceptBase.cc
[offline]>Successfully connected to server
[localhost:4001]>yes
[localhost:4001]>
>>> Telling ./SameGenRule.sml.txt
This is CBShell, the command line interface to ConceptBase.cc
[offline]>Successfully connected to server
[localhost:4001]>yes
[localhost:4001]>
>>> Telling ./TreeExample.sml.txt
This is CBShell, the command line interface to ConceptBase.cc
[offline]>Successfully connected to server
[localhost:4001]>yes
[localhost:4001]>
...
[localhost:4001]>yes
[localhost:4001]>
>>> Telling ./WinNim.sml.txt
This is CBShell, the command line interface to ConceptBase.cc
[offline]>Successfully connected to server
[localhost:4001]>yes
[localhost:4001]>
>>> Telling ./WinTicTacToe.sml.txt
This is CBShell, the command line interface to ConceptBase.cc
[offline]>Successfully connected to server
[localhost:4001]>yes
[localhost:4001]>
>>> Telling ./links.sml.txt
This is CBShell, the command line interface to ConceptBase.cc
[offline]>Successfully connected to server
[localhost:4001]>yes
[localhost:4001]>
