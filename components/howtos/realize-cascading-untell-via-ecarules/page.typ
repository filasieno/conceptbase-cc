...
```bash
nix build .#checks.x86_64-linux.realize-cascading-untell-via-ecarules
```
...
{* These rules will delete links to/from a deleted object *}
{* Hence, instead of following all dependencies, these    *}
{* rules will do most of the work.                        *}
...
ECArule overwriteattributee1 with
  ecarule
    r1: $ a,a1,ac/attributee x,y,y1/Proposition n/Label 
         ON Tell (a1 in attributee)
         IF `P(a1,x,n,y1) and P(a,x,n,y) and In_s(a,ac)
         DO Untell (a in ac)
        $
...
ECArule overwriteattributee2 with
  ecarule
    r1: $ a,a1/attributee ac/attributee x,y,y1/Proposition n/Label 
         ON Tell (a1 in attributee)
         IF `P(a1,x,n,y1) and P(a,x,n,y)
         DO Untell (a in attributee)
        $
...
# of the transaction. Here, only ECArule overwriteattributee2
...
```text
=== HOW-TO: realize-cascading-untell-via-ecarules (asset validation + smoke) ===
