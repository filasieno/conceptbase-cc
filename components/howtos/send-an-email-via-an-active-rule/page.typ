...
```bash
nix build .#checks.x86_64-linux.send-an-email-via-an-active-rule
```
...
```telos
A1 in HTML_Form with
  feature
    form_type: "AbstractSubmission";
    memberPC: "yes";
    content: "This is an example abstract submission ..."
end
...
```text
=== HOW-TO: send-an-email-via-an-active-rule (asset validation + smoke) ===
