...
```bash
nix build .#checks.x86_64-linux.manage-access-rights-to-modules-and-other-resources
```

== Input

=== `Individual workspaces/OwnerProtect.sml.txt`
...
```telos
{*
 * File: RightsExample.sml
 * Author: M. Jeusfeld (19-Feb-2004) 
 * -----------------------------------------------------------------
 * this example defines CB_Group, some rules for deriving read/write 
 * permissions and the all important query CB_Permitted that is
 * to be used by the CBserver.                
 * Requires ConceptBase 6.1 (19-Feb-2004) or later!
*}


{* Resource, CB_Operation, CB_User are predefined in system model module.sml *}
...
{* example of some module owned by someone
M2 in Module end

CB_User jonny with
  owner_resource r1: M2
end
*}
...
=== `Rights over modules/01-RightsOverModules.sml.txt`

```telos
{*
 * File: RightsOverModules.sml
 * Author: M. Jeusfeld (25-Feb-2004, 12-Dec-2011) 
 * -----------------------------------------------------------------
 * Example rights model for all to be told to module 'oHome'.
 * Be sure that you start the CBserver with option -s 2 to
...
=== `Rights over modules/02-RightsMjeusfeld.sml.txt`
...
=== `Rights over modules/03-RightsMjonny.sml.txt`

```telos
{*
 * File: RightsMjonny.sml
 * Author: M. Jeusfeld (24-Feb-2004) 
 * -----------------------------------------------------------------
 * to be told to module oHome-Mjonny after having loaded RightsOverModules.sml
*}


{*
Very simple: if a user has has ~res as home module then she can perform write operations.
If she is just a CB_User then she can do read operations.
*}
...
```text
=== HOW-TO: manage-access-rights-to-modules-and-other-resources (asset validation + smoke) ===
