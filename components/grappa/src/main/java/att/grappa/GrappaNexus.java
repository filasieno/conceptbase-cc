 ...
 *
 * @version $Id: 648c7532d9bc633857ef9dac4f48215126c05d7f $
...
public class GrappaNexus implements GrappaConstants, Cloneable, ImageObserver, Observer, Shape
{
    ...
    /**
     * Indicates if the area bounding the element text should be filled/outlined along with the element when being
     * drawn. By default, it is not.
     *
    ...
    /**
     * Return status of image loading. Returns true whenever an image has begun loading for this element, but has not
     * yet completed.
     *
    ...
    /**
     * Update the shape information for the underlying element. For nodes, the <I>distortion</I>, <I>height</I>,
     * <I>orientation</I>, <I>peripheries</I>, <I>pos</I>, <I>rotation</I>, <I>shape</I>, <I>sides</I>, <I>skew</I> and
     * <I>width</I> attributes are examined. For edges, the <I>pos</I> attribute is examined. For subgraph, the bounding
     * box is recomputed.
     */
    public void updateShape()
    {
        ...
        switch (this.element.getType()) {
            case NODE:
                ...
                switch (type) {
                    case CUSTOM_SHAPE:
                        ...
                        try {
                            ...
                            if (e instanceof InvocationTargetException) {
                                ...
                                throw new IllegalArgumentException("cannot instantiate custom shape '" + custom
                                    + "' for node '" + node.getName() + "' because2: "
                                    + ((UndeclaredThrowableException) e).getUndeclaredThrowable().getMessage());
                            } else {
                                throw new IllegalArgumentException("cannot instantiate custom shape '" + custom
                                    + "' for node '" + node.getName() + "' because3: " + e.getMessage());
                            }
                        }
                        this.shapeType = CUSTOM_SHAPE;
                        break;
                    case BOX_SHAPE:
                        if ((Distortion == null || distortion == 0)
                            ...
                            this.shapeType = BOX_SHAPE;
                            if (Grappa.centerPointNodes) {
                                this.shape =
                                    new Rectangle2D.Double(pos.x - (width / 2.0), pos.y - (height / 2.0), width, height);
                            } else {
                                this.shape = new Rectangle2D.Double(pos.x, pos.y, width, height);
                            }
                            if (Peripheries != null && peripheries > 1) {
                                path = new GeneralPath(this.shape);
                                    ...
                                    if (Grappa.centerPointNodes) {
                                        path.append
                                            (
                                                ...
                                                (
                                                    (pos.x - width / 2.0) + i * PERIPHERY_GAP,
                                                    (pos.y - height / 2.0) + i * PERIPHERY_GAP,
                                                    width - 2 * i * PERIPHERY_GAP,
                                                    height - 2 * i * PERIPHERY_GAP
                                                ),
                                                false
                                            );
                                    } else {
                                        path.append
                                            (
                                                new Rectangle2D.Double
                                                (
                                                    pos.x + i * PERIPHERY_GAP,
                                                    pos.y + i * PERIPHERY_GAP,
                                                    width - 2 * i * PERIPHERY_GAP,
                                                    height - 2 * i * PERIPHERY_GAP
                                                ),
                                                false
                            ...
                            this.shapeType = BOX_SHAPE | GRAPPA_SHAPE;
                            if (Grappa.centerPointNodes) {
                                this.shape =
                                    new GrappaShape(this.shapeType, pos.x, pos.y, width, height, sides, peripheries,
                                        distortion, skew, orientation, this.style.rounded, this.style.diagonals, null);
                            } else {
                                this.shape =
                                    new GrappaShape(this.shapeType, pos.x + (width / 2.0), pos.y + (height / 2.0),
                                        width,
                                        height, sides, peripheries, distortion, skew, orientation, this.style.rounded,
                                        this.style.diagonals, null);
                            }
                        }
                        break;
                    case ROUNDEDBOX_SHAPE:
                        if ((Distortion == null || distortion == 0)
                            ...
                            &&
                            (Orientation == null || orientation == 0)) {
                            this.shapeType = ROUNDEDBOX_SHAPE;
                            if (Grappa.centerPointNodes) {
                                this.shape =
                                    new RoundRectangle2D.Double(pos.x - (width / 2.0), pos.y - (height / 2.0), width,
                                        height, arcWidthFactor * width, arcHeightFactor * height);
                            } else {
                                this.shape =
                                    new RoundRectangle2D.Double(pos.x, pos.y, width, height, arcWidthFactor * width,
                                        ...
                                        path.append
                                            (
                                                ...
                                                (
                                                    ...
                                                    width - 2 * i * PERIPHERY_GAP,
                                                    height - 2 * i * PERIPHERY_GAP,
                                                    arcWidthFactor * (width - 2 * i * PERIPHERY_GAP),
                                                    arcHeightFactor * (height - 2 * i * PERIPHERY_GAP)
                                                ),
                                                false
                                            );
                                    } else {
                            ...
                            this.shapeType = ROUNDEDBOX_SHAPE | GRAPPA_SHAPE;
                            if (Grappa.centerPointNodes) {
                                this.shape =
                                    new GrappaShape(this.shapeType, pos.x, pos.y, width, height, sides, peripheries,
                                        distortion, skew, orientation, this.style.rounded, this.style.diagonals, null);
                    ...
                    case OVAL_SHAPE:
                        if ((Distortion == null || distortion == 0)
                            ...
                            if (Grappa.centerPointNodes) {
                                this.shape =
                                    new Ellipse2D.Double(pos.x - (width / 2.0), pos.y - (height / 2.0), width, height);
                            } else {
                                this.shape = new Ellipse2D.Double(pos.x, pos.y, width, height);
                            }
                            if (Peripheries != null && peripheries > 1) {
                                path = new GeneralPath(this.shape);
                                for (int i = 1; i < peripheries; i++) {
                            ...
                            this.shapeType = OVAL_SHAPE | GRAPPA_SHAPE;
                            if (Grappa.centerPointNodes) {
                                this.shape =
                                    new GrappaShape(this.shapeType, pos.x, pos.y, width, height, sides, peripheries,
                                        distortion, skew, orientation, this.style.rounded, this.style.diagonals, null);
                    ...
                    case MDIAMOND_SHAPE:
                    case MSQUARE_SHAPE:
                        this.shapeType = type | GRAPPA_SHAPE;
                        if (Grappa.centerPointNodes) {
                            this.shape =
                                new GrappaShape(this.shapeType, pos.x, pos.y, width, height, sides, peripheries,
                                    distortion,
                                    skew, orientation, this.style.rounded, this.style.diagonals, null);
                        } else {
                            this.shape =
                                new GrappaShape(this.shapeType, pos.x + (width / 2.0), pos.y + (height / 2.0), width,
                                    height, sides, peripheries, distortion, skew, orientation, this.style.rounded,
                                    this.style.diagonals, null);
                        }
                        break;
                    ...
                    case MRECORD_SHAPE:
                        ...
                        if (Grappa.centerPointNodes) {
                            this.shape =
                                new GrappaShape(this.shapeType, pos.x, pos.y, width, height, sides, peripheries,
                                    distortion,
                                    skew, orientation, this.style.rounded, this.style.diagonals, rects);
                        } else {
                            this.shape =
                                new GrappaShape(this.shapeType, pos.x + (width / 2.0), pos.y + (height / 2.0), width,
                                    height, sides, peripheries, distortion, skew, orientation, this.style.rounded,
                                    this.style.diagonals, rects);
                        }
                        break;
                    default:
                ...
                if (Rotation != null && rotation != 0 && this.shape != null) {
                    ...
                    if (Grappa.centerPointNodes) {
                        this.shape =
                            AffineTransform.getRotateInstance(theta, pos.x, pos.y).createTransformedShape(this.shape);
                    } else {
                        this.shape =
                            AffineTransform.getRotateInstance(theta, pos.x + (width / 2.0), pos.y + (height / 2.0))
            ...
            case EDGE:
                ...
                this.shapeType = LINE_SHAPE;

                if (this.element.getSubgraph().isCluster() && this.element.getSubgraph().grappaNexus != null) {
                    this.element.getSubgraph().grappaNexus.dirty = true;
                }

                if ((this.shape = (Shape) edge.getAttributeValue(POS_ATTR)) == null) {
                    Integer attr_type = (Integer) (edge.getAttributeValue(DIR_ATTR));

                    edge.direction =
                        (attr_type != null ? attr_type.intValue() : (edge.getGraph().isDirected()
                            ? GrappaLine.TAIL_ARROW_EDGE : GrappaLine.NONE_ARROW_EDGE));

                    // create a default straight line connecting the two
                    // node centers
                    edge.setAttribute(
                        "pos",
                        new GrappaLine(new GrappaPoint[] { (GrappaPoint) (edge.getTail().getAttributeValue(POS_ATTR)),
                        (GrappaPoint) (edge.getHead().getAttributeValue(POS_ATTR)) }, edge.direction)
                        );
                    this.shape = (Shape) edge.getAttributeValue(POS_ATTR);
                }
                break;
            case SUBGRAPH:
                ...
                GrappaSize minSize = (GrappaSize) this.element.getAttributeValue(MINSIZE_ATTR);
                if (minSize != null) {
                    if (sgbox == null) {
                        sgbox = new java.awt.geom.Rectangle2D.Double(0, 0, minSize.getWidth(), minSize.getHeight());
                    } else {
                        sgbox.add(new java.awt.geom.Rectangle2D.Double(sgbox.getCenterX() - (minSize.getWidth() / 2.0),
                            sgbox.getCenterY() - (minSize.getHeight() / 2.0), minSize.getWidth(), minSize.getHeight()));
                    }
                }
                GrappaBox minBox = (GrappaBox) this.element.getAttributeValue(MINBOX_ATTR);
                if (minBox != null) {
                    if (sgbox == null) {
                        sgbox = new java.awt.geom.Rectangle2D.Double(minBox.x, minBox.y, minBox.width, minBox.height);
                    } else {
                        sgbox
                            .add(new java.awt.geom.Rectangle2D.Double(minBox.x, minBox.y, minBox.width, minBox.height));
                    }
                }
                if (sgbox == null) {
                ...
                break;
            default:
                throw new IllegalArgumentException("unrecognized element type (" + this.element.getType() + ") for "
    ...
    /**
     * Update the shape information for the underlying element. The <I>style</I> attribute is examined.
     */
    public void updateStyle()
    {
        long thisStyleUpdate = System.currentTimeMillis();
        if ((this.style = (GrappaStyle) this.element.getAttributeValue(STYLE_ATTR)) == null) {
        ...
        if (this.style.font_style != null
            &&
            this.style.font_style != (Integer) this.element.getAttributeValue(FONTSTYLE_ATTR)) {
            this.element.setAttribute(FONTSTYLE_ATTR, this.style.font_style);
    ...
    /**
     * Update the text information for the underlying element. The <I>fontcolor</I>, <I>fontname</I>, <I>fontsize</I>,
     * <I>fontstyle</I>, and <I>label</I> attributes are examined. The <I>lp</I> attribute is also examined for edges
     * and subgraphs.
     */
    public void updateText()
    {
        ...
        String labelAttr = (String) this.element.getAttributeValue(LABEL_ATTR);
        ...
        if (this.element.isEdge()) {
            if ((headstr = (String) this.element.getAttributeValue(HEADLABEL_ATTR)) != null
                && (attr = this.element.getLocalAttribute(HEADLP_ATTR)) != null) {
            ...
            if ((tailstr = (String) this.element.getAttributeValue(TAILLABEL_ATTR)) != null
                && (attr = this.element.getLocalAttribute(TAILLP_ATTR)) != null) {
        ...
        if (labelAttr != null || hasEdgeLabel) {
            if (this.element.isNode()
                &&
                (
                this.shapeType == (RECORD_SHAPE | GRAPPA_SHAPE)
                ||
                this.shapeType == (MRECORD_SHAPE | GRAPPA_SHAPE)
                )
                &&
                labelAttr.indexOf('|') < 0
                &&
            ...
            if (hasEdgeLabel || labelAttr.length() > 0) {
                ...
                String fontname = (String) this.element.getAttributeValue(FONTNAME_ATTR);
                Integer fontstyle = (Integer) this.element.getAttributeValue(FONTSTYLE_ATTR);
                Integer fontsize = (Integer) this.element.getAttributeValue(FONTSIZE_ATTR);
                Integer fontadj =
                    (Integer) (this.element.getGraph()).getGrappaAttributeValue(GRAPPA_FONTSIZE_ADJUSTMENT_ATTR);
                ...
                if (this.element.isNode()
                    ...
                    if (labelAttr != null) {
                        if (Grappa.autoPositionNodeLabel && this.element.isNode()) {
                            lps[0] = ((Node) this.element).getCenterPoint();
                        } else if ((attr = this.element.getLocalAttribute(LP_ATTR)) == null
                            || ((sg = this.element.getSubgraph()) != null && attr == sg.getLocalAttribute(LP_ATTR))) {
                            Rectangle2D lbox;

                            if ((lbox = (Rectangle2D) this.element.getAttributeValue(BBOX_ATTR)) == null) {
                                lbox = this.bbox;
                            }

                            if (this.element.isSubgraph() && lbox != null) {
                                lps[0] =
                                    new GrappaPoint(lbox.getX() + lbox.getWidth() / 2.0, (Grappa.labelGraphBottom
                                        ? lbox.getMaxY() : lbox.getMinY()));
                                this.element.setAttribute(LP_ATTR, lps[0].clone());
                            } else {
                                lps[0] = null;
                            }
                        } else {
                    ...
                    if (labels[l] != null && lps[l] != null && labels[l].length() > 0) {
                        ...
                        for (i = 0; i < array.length; i++) {
                            if (array[i] == '\\'
                                ...
                                bnds[lines] =
                                    tfont.getStringBounds(tstr[offset + lines], this.element.getGraph().REFCNTXT);
                                mtrc[lines] =
                                    tfont.getLineMetrics(tstr[offset + lines], this.element.getGraph().REFCNTXT);
    ...
    /**
     * Update the decoration information for the underlying element. The <I>color</I> and <I>fontcolor</I> attributes
     * are examined. For edges, the <I>dir</I> attribute is examined.
     */
    public void updateDecoration()
    {
        long thisDecorationUpdate = System.currentTimeMillis();
        this.color = (Color) (this.element.getAttributeValue(COLOR_ATTR));

        @SuppressWarnings("unchecked")
        java.util.List<Color> fillcolorList = (java.util.List<Color>) this.element.getAttributeValue(FILLCOLOR_ATTR);
        if (fillcolorList != null && fillcolorList.size() > 0) {
            this.fillcolor = fillcolorList.get(0);
        } else {
            this.fillcolor = null;
        }

        this.font_color = (Color) (this.element.getAttributeValue(FONTCOLOR_ATTR));
    ...
    public void updateImage()
    {
        ...
        String path = (String) (this.element.getAttributeValue(IMAGE_ATTR));
    ...
    private void bboxCheckSet()
    {
        ...
        if (newbox == null) {
            if (this.element.isSubgraph() && ((Subgraph) this.element).countOfElements(SUBGRAPH | NODE | EDGE) == 0) {
        ...
        if ((oldbox == null && newbox != null)
            ...
            Subgraph prnt = this.element.getSubgraph();
            while (prnt != null) {
        ...
        try {
            ...
            if (this.shape != null) {
                if (this.shapeType == LINE_SHAPE) {
                    copy.shape = (Shape) ((GrappaLine) this.shape).clone();
                } else if (this.shapeType == BOX_SHAPE) {
                    copy.shape = (Shape) ((Rectangle2D) this.shape).clone();
                } else if (this.shapeType == ROUNDEDBOX_SHAPE) {
                    copy.shape = (Shape) ((RoundRectangle2D) this.shape).clone();
                } else if (this.shapeType == OVAL_SHAPE) {
                    copy.shape = (Shape) ((Ellipse2D) this.shape).clone();
                } else if ((this.shapeType & GRAPPA_SHAPE) != 0) {
                    copy.shape = (Shape) ((GrappaShape) this.shape).clone();
                } else {
                    copy.shape = (Shape) ((GeneralPath) this.shape).clone();
                }
            }
    ...
    public boolean contains(double x, double y)
    {
        ...
        if (this.textArea != null && !contains && !this.clearText && this.drawText
            &&
            (
            (this.element.isNode() && this.element.getGraph().getShowNodeLabels())
                ||
                (this.element.isEdge() && this.element.getGraph().getShowEdgeLabels())
                ||
            (this.element.isSubgraph() && this.element.getGraph().getShowSubgraphLabels())
            )) {
    ...
    public boolean intersects(double x, double y, double width, double height)
    {
        ...
        if (this.textArea != null && !intersects && !this.clearText && this.drawText
            &&
            (
            (this.element.isNode() && this.element.getGraph().getShowNodeLabels())
                ||
    ...
    /**
     * This method is called whenever the observed object is changed. When certain observed attributes (attributes of
     * interest) are changed, this method will update the GrappaNexus as needed.
     *
     * @param obs the Observable must be an Attribute
     * @param arg either a Long giving the update time of the Attribute as returned by System.getTimeInMillis() or it is
    ...
    public void update(java.util.Observable obs, Object arg)
    {
        ...
        if (!(obs instanceof Attribute)) {
            throw new IllegalArgumentException("expected to be observing attributes only (obs) for \""
                + this.element.getName() + "\"");
        }
        ...
        if (arg instanceof Long) {
            ...
            switch (this.element.getType()) {
                case NODE:
                    if ((POS_HASH == attrHash && POS_ATTR.equals(attrName))
                        ...
                        if (this.lastShapeUpdate < thisUpdate) {
                            updateShape();
                            if (Grappa.autoPositionNodeLabel) {
                                updateText();
                            }
                        }
                    } else if ((LABEL_HASH == attrHash && LABEL_ATTR.equals(attrName))
                        ||
                        (
                        ...
                        if (this.lastTextUpdate < thisUpdate) {
                            updateText();
                        }
                    } else if ((STYLE_HASH == attrHash && STYLE_ATTR.equals(attrName))) {
                        if (this.lastStyleUpdate < thisUpdate) {
                            updateStyle();
                        }
                    } else if ((COLOR_HASH == attrHash && COLOR_ATTR.equals(attrName))
                        ||
                        (FONTCOLOR_HASH == attrHash && FONTCOLOR_ATTR.equals(attrName))) {
                        if (this.lastDecorationUpdate < thisUpdate) {
                            updateDecoration();
                        }
                    } else if ((IMAGE_HASH == attrHash && IMAGE_ATTR.equals(attrName))) {
                        if (this.lastImageUpdate < thisUpdate) {
                            updateImage();
                        }
                    } else {
                        throw new InternalError("update called for \"" + this.element.getName()
                ...
                case SUBGRAPH:
                    ...
                    } else if ((MINBOX_HASH == attrHash && MINBOX_ATTR.equals(attrName))
                        ||
            ...
            throw new InternalError("update called for shape of element \"" + this.element.getName()
    ...
    void draw(java.awt.Graphics2D g2d)
    {
        if (this.shape instanceof CustomRenderer) {
            ((CustomRenderer) this.shape).draw(g2d);
        } else {
            g2d.draw(this);
        }
    }
    ...
    {
        if (this.shape instanceof CustomRenderer) {
            ((CustomRenderer) this.shape).fill(g2d);
        } else {
            g2d.fill(this);
        }
    }
    ...
    void drawImage(java.awt.Graphics2D g2d)
    {
        ...
        if (this.image != null) {
            if (this.shape instanceof CustomRenderer) {
                ((CustomRenderer) this.shape).drawImage(g2d);
            } else {
