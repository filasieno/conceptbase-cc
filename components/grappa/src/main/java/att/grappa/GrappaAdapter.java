...
/**
 * A convenience class that implements the GrappaListener interface for handling mouse-related activity that occurs on a
 * graph. This particular GrappaListener implementation allows the following interactions with a displayed graph:
 * <UL>
 ...
 * <LI>shift-button 1 drag, when starting in one node and ending in another, will create an edge;
 ...
 * <LI>clear selection, enclose the selected items in a new subgraph, preview deletion, cancel preview and perform
 * deletion, if at least one item is selected;
 * <LI>a tooltip on/off toggle option.
 * </UL>
 * </UL>
 *
 * @version $Id: e7baaaddd05d666b5878d4eac0099fbe3a1a61da $
...
public class GrappaAdapter implements GrappaConstants, GrappaListener, ActionListener
{
                    ...
                    if (outline == null) {
                        if (pressedElem == elem && pt.distance(pressedPt) < 5) {
                            ...
                            if (attr == null || attr.getValue().equals("\\N")) {
                                attrs =
                                    new Attribute[] {
                                    new Attribute(GrappaConstants.NODE, POS_ATTR, pt),
                                    new Attribute(GrappaConstants.NODE, LABEL_ATTR, "Node"
                                        + subg.getGraph().getId(GrappaConstants.NODE)) };
                            } else {
                                attrs = new Attribute[] { new Attribute(GrappaConstants.NODE, POS_ATTR, pt) };
                            }
                            Element el = subg.createElement(GrappaConstants.NODE, null, attrs);
                            if (el != null) {
                                el.buildShape();
                                subg.getGraph().repaint();
                            }
                            ...
                            Attribute[] attrs =
                                new Attribute[] { new Attribute(GrappaConstants.EDGE, POS_ATTR, new GrappaLine(
                                    new GrappaPoint[] { ((Node) pressedElem).getCenterPoint(),
                                    ((Node) elem).getCenterPoint() }, subg.getGraph().isDirected()
                                        ? GrappaLine.TAIL_ARROW_EDGE : GrappaLine.NONE_ARROW_EDGE)) };
                            Element el = subg.createElement(GrappaConstants.EDGE, info, attrs);
                            if (el != null) {
                                el.buildShape();
                                subg.getGraph().repaint();
                            }
     ...
     * @param modifiers mouse modifiers in effect
     * @param panel specific panel where the action occurred
     * @return the tip to be displayed or null; in this implementation, if the mouse is in a graph element that has its
     *         <I>tip</I> attribute defined, then that text is returned. If that attribute is not set, the element name
