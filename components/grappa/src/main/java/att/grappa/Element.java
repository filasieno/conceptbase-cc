 ...
 *
 * @version $Id: b3df0da4a49af25ef5d6462c6ed080ea28840799 $
...
public abstract class Element implements GrappaConstants
{
    ...
    /*
     * These (deleteCalled and busy) used by reserve/release/setDelete to queue a delete request during a critical
     * function (e.g., drawing) and to define the start and end of that critical function.
     */
     ...
     * an Element without the need for extending the class.
     */
    public Object object;

    /**
     * Indicates whether this element should be considered visible or not. If not visible, it will not be drawn and will
     * not be selected by a mouse click (if the supplied selection methods are used). Note that there is redundancy here
     * with the <i>invis</i> component of the <i>style</i> attribute. This value must be true and <i>invis</i> must be
     * false for the element to be visible.
     */
    ...
    /**
     * Indicates whether only the list of attributes found in the PRINTLIST_ATTR should be printed. The default is
     * false.
     */
    public static boolean usePrintList = false;

    /**
     * A convenience variable, not used by Grappa, but available to keep track of or otherwise mark graph elements when
     * traversing a graph.
     */
    public int counter = 0;

    /**
     * Determines the type of highlighting to apply, if any, when drawing. Currently recognized choices are
     * SELECTION_MASK and DELETION_MASK.
     */
    ...
    /**
     * Boolean to indicate if all of this element's attributes should be printed. Either this flag or the
     * <TT>elementPrintAllAttributes</TT> can turn on printing of all attributes.
     *
    ...
    /**
     * Boolean to indicate if the default attributes associated with this element should be printed. Naturally, this
     * option only is effective if the element is a subgraph.
     *
     * @see Subgraph#printSubgraph
    ...
    /**
     * Element constructor needed only during init phase of <A HREF="att.grappa.Graph.html">Graph</A> class. Since the
     * Element class is abstact, it cannot be instantiated directly.
     */
    protected Element()
    {
    ...
    /**
     * Element constructor used during init phase of the <A HREF="att.grappa.Node.html">Node</A>, <A
     * HREF="att.grappa.Edge.html">Edge</A> and <A HREF="att.grappa.Subgraph.html">Subgraph</A> classes. Since the
     * Element class is abstact, it cannot be instantiated directly.
     *
    ...
    /**
     * Intended to be a subclass-specific name generating method. Used by edges and when nodes or graphs are created
     * without an explicit name. Note that graphs and nodes should also have a setName that takes an explicit name as an
     * argument.
     */
     ...
     *
     * @return true, unless a delete request has already been queued for this Element.
     */
    ...
    private synchronized boolean setReserved(boolean state, boolean isDelete)
    {
        if (isDelete) {
            if (state) {
                ...
                this.deleteCalled = this.busy = false;
                return (true);
            }
        } else if (state) {
            ...
            if (!this.deleteCalled) {
                ...
                this.busy = this.deleteCalled = false;
                delete();
            }
            return (true);
        }
    ...
    protected void attrNotOfInterest(String name)
    {
        ...
        if (this.grappaNexus != null) {
            Attribute attr = getAttribute(name);
            if (attr != null) {
                attr.deleteObserver(this.grappaNexus);
            }
        }
        this.attrsOfInterest.remove(name);
    }
    ...
    /**
     * Sets or creates an attribute for this element from the attribute supplied. The storage key is the attribute name.
     * If the value portion of the supplied attribute is null, then the attribute will be removed from the element.
     *
     * @param attr the attribute from which to set the element's attribute.
     * @return the value of the (local) attribute previously stored under the same name
     */
    ...
    /**
     * Sets or creates an attribute for this element from the supplied arguments. The storage key is the attribute name.
     * If the value argument is null, then the attribute will be removed from the element.
     *
    ...
    public Object setAttribute(String name, Object value)
    {
        ...
        if (crntAttr == null) {
            ...
            this.attributes.put(name, (crntAttr = new Attribute(getType(), name, value)));
    ...
    /*
     * Removes the named attribute from the (local) attribute table and applies the default attribute (if any)
    ...
    /**
     * Sets or creates a default attribute for this element type within the containing subgraph of this element from the
     * supplied arguments. The storage key is the attribute name. If the value argument is null, then the attribute will
     * be removed from the subgraph.
     *
     * @param name the attribute name
     * @param value the attribute value
     * @return the value of the (default) attribute previously stored under the same name
     */
    ...
    /**
     * Sets or creates a default attribute of the specified type within the containing subgraph of this element from the
     * supplied arguments. The storage key is the attribute name. If the value argument is null, then the attribute will
     * be removed from the subgraph.
     *
    ...
    /**
     * Sets or creates a default attribute for this element type within the containing subgraph of this element from the
     * supplied arguments. The storage key is the attribute name. If the value portion of the supplied attribute is
     * null, then the attribute will be removed from the subgraph.
     *
    ...
    /**
     * Get only the corresponding local attribute for the specified key. A local attribute is one associated directly
     * with this element as opposed to a subgraph ancestor.
     *
    ...
    /**
     * Get only the corresponding local attribute for the specified key if it is not inherited from the parent,
     * otherwise return null. Sometimes a local attribute will be set, but it will be a pointer to the parent value.
     * This method distinguishes that case.
     *
     * @param key the search key for the corresponding attribute.
     * @return the local Attribute object matching the key or null if it is not defined or it is a pointer to the parent
     *         attribute..
     */
    ...
    /**
     * Get only the value of the corresponding local attribute for the specified key if the attribute is not inherited
     * from the parent, otherwise return null. Sometimes a local attribute will be set, but it will be simply a pointer
     * to the parent attribute. This method distinguishes that case.
     *
     * @param key the search key for the corresponding attribute.
     * @return the value of the local Attribute object matching the key or null if it is not defined or it is a pointer
    ...
    /**
     * Get the Attribute of this Element for the specified key. Search first local, then default attributes until a
     * match is found.
     *
    ...
    /**
     * Get the Attribute value of this Element for the specified key. Search first local, then default attributes until
     * a match is found.
     *
     * @param key the search key for the attribute.
    ...
    {
        this.idKey = Graph.idMapKey(type, getGraph().nextId(type));
    }
    ...
    /*
     * Print attributes to given stream. A square open bracket prefix and closed bracket suffix enclose the attributes,
     * but are printed only if there are any attributes to print. The supplied indent determines the indentation of the
     * final bracket (it is assumed the element name has already printed to the output stream.
     * @param out the print stream for output.
    ...
    private void printAttributes(PrintWriter out, String outerIndent)
    {
        String indent = new String(getGraph().getIndent());
        String prefix = " [";
        String suffix = GrappaConstants.NEW_LINE + outerIndent + "];";
        Attribute attr;
        String key;
        boolean first = true;
        ...
        if (attrs != null) {
            while (attrs.hasMoreElements()) {
                ...
                if (attr != null
                    && (Grappa.elementPrintAllAttributes || this.printAllAttributes || !attr
     ...
     * @return the input string, possibly enclosed in double quotes and with internal double quotes protected.
     */
     ...
     * @param uplow set true to indicate the return value should be leading-capitalized, otherwise lower-case is
     *            returned
     ...
     * @return the input string, with non-alphanumerics removed and alphabetics are converted to lower-case.
     */
    ...
    /**
     * Sets the conversion type of a user-defined attribute. Unless provided for a specific attribute name, the
     * attribute value will only be treated as a string. When provided, the string value of the attribute will be
    ...
    public static int setUserAttributeType(String attrname, int attrtype)
    {
        ...
        if (type == null) {
            throw new IllegalArgumentException("supplied type for attribute (" + attrname
    ...
    /**
     * Creates and populates the GrappaNexus object for this element. The GrappaNexus object provides bounding and
     * drawing information for the element based on the element's attributes.
     */
     ...
     *
     * @param steps when negative, the search is exhaustive; otherwise the search stops after the number of steps
     *            indicated
     * @return a Vector of Vector, the ith element of which gives the search results for step i. Reading the vector in
