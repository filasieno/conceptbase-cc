 ...
 *
 * @version $Id: 1038d199bff480eab14addce82a90f05c8a19541 $
...
public class Graph extends Subgraph
{
    ...
    /**
     * When filtering a graph (see GrappaSupport.filterGraph) or whenever edge names are desired as part of the output
     * of a printGraph call, set this value to true. The edge name will be included among edge attributes in the print
     * output as '__nAmE__'. filterGraph automatically handles setting this variable, but users writing their own
     * version of filterGraph will need it.
     */
     ...
     */
    public final java.awt.font.FontRenderContext REFCNTXT = new java.awt.font.FontRenderContext(IDENTXFRM,
        Grappa.useAntiAliasing, Grappa.useFractionalMetrics);
    ...
    private void initialize(String graphName)
    {
        ...
        setGraph(this);
        setSubgraph(null);
        setIdKey(GrappaConstants.SUBGRAPH);
        addIdMapping(this);
        setName(graphName);
    ...
    /**
     * Sets or unsets indication that paint requests should be done within a synchronized wrapper that prevents
     * concurrent paints and any paints between calls to the dropcloth method.
     *
    ...
    /**
     * Sets and unsets a flag in a synchronized manner so that during the period that the flag is set, painting will not
     * occur.
     *
     * @param block value to which to set the indicator flag
     * @param auto when block is false, setting this parameter true will request a repaint() if any paint requests
     *            arrived while the dropcloth was laid out.
    ...
    private synchronized boolean setBlocked(boolean state, boolean isPaint, boolean repaint)
    {
        if (isPaint) {
            if (state) {
                ...
                this.paintCalled = this.busy = false;
                return (true);
            }
        } else if (state) {
            if (this.paintCalled) {
            ...
            if (!this.paintCalled) {
                ...
                this.busy = this.paintCalled = false;
                if (repaint) {
                    repaint();
                }
            }
    ...
    /**
     * Sets a Grappa package attribute. A Grappa package attribute is one specific to Grappa (for example, a display
     * color) rather than an attribute that relates to a graph.
     *
     * @param key the search key for the corresponding attribute.
    ...
    public Object setGrappaAttribute(String key, String value) throws IllegalArgumentException
    {
        ...
        if (oldValue == null) {
            if (!validGrappaAttributeKey(key)) {
                throw new IllegalArgumentException(GrappaConstants.PKG_UPLOW + " attribute key must use \""
            ...
            this.grattributes.put(key, new Attribute(SYSTEM, key, value));
        }
        return oldValue.getValue();
    }

    /**
     * Returns the attribute conversion type for the supplied attribute name. Only graph global specific attribute
     * name/type mappings are checked.
     *
    ...
    /**
     * Check if the given key has a format consistent with Grappa package attribute keys. A Grappa package key starts
     * with Grappa.PKG_LOWER.
     *
    ...
    {
        return (key != null && key.startsWith(GrappaConstants.PKG_LOWER) && key.length() > GrappaConstants.PKG_LOWER
            .length());
    }

    /**
     * Gets a graph default attribute. A graph default attribute determines basic graph characteristics initially (e.g.,
     * node shape).
     *
    ...
    {
        long value = (id);
        int tval = (type & (GrappaConstants.NODE | GrappaConstants.EDGE | GrappaConstants.SUBGRAPH));
        if (tval == 0) {
    ...
    static int idKeyType(Long idKey)
    {
        ...
        return (int) (value & (GrappaConstants.NODE | GrappaConstants.EDGE | GrappaConstants.SUBGRAPH));
    }
    ...
    {
        long value = idKey.longValue();
        return (int) (value >>> GrappaConstants.TYPES_SHIFT);
    }
    ...
    /**
     * Reset this graph by removing all its elements and re-initiailizing its internal variables and possibly changing
     * its name, directedness and strictness.
     */
    ...
    /**
     * Print the supplied message and exception information to the error output. Nothing happens if the error output is
     * set to null.
     *
     * @param msg the message to print on the error output.
    ...
    /**
     * Builds any GrappaNexus object not already built and rebuilds those that already exist for all elements in this
     * graph.
     */
    public void resync()
    {
