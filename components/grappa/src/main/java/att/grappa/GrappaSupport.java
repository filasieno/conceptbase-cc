 ...
 *
 * @version $Id: cc96c4edf963d16e2564e14c3a67f8d4a1137326 $
...
public abstract class GrappaSupport implements GrappaConstants
{
    ...
    /**
     * Converts a string to an integer edge direction. The string is first canonicalized (converted to lower case and
     * non-alphanumerics are removed) then compared to none, back, forward or both. A match returns
     * GrappaLine.NONE_ARROW_EDGE, GrappaLine.HEAD_ARROW_EDGE, GrappaLine.TAIL_ARROW_EDGE or GrappaLine.BOTH_ARROW_EDGE,
     * respectively. When there is no match, GrappaLine.NONE_ARROW_EDGE is returned.
     *
    ...
    /**
     * Converts an integer edge direction value to a string representation. Only GrappaLine.NONE_ARROW_EDGE,
     * GrappaLine.HEAD_ARROW_EDGE, GrappaLine.TAIL_ARROW_EDGE and GrappaLine.BOTH_ARROW_EDGE are understood, all others
     * are taken to mean GrappaLine.NONE_ARROW_EDGE.
     *
    ...
    /**
     * Converts a string to an integer font style. The string is first canonicalized (converted to lower case and
     * non-alphanumerics are removed) then compared to italic, bold or bolditalic. A match returns Font.ITALIC,
     * Font.BOLD, or a bitwise OR-ing of the two, respectively. When there is no match, Font.PLAIN is returned.
     *
    ...
    /**
     * Converts an integer font style value to a string representation. Only Font.ITALIC, Font.BOLD, and
     * (Font.BOLD|Font.ITALIC) are understood, all others are taken to mean Font.PLAIN.
     *
     ...
     * Creates a GrappaBox from the coordinates of any two opposing corners.
     *
     * @param box if non-null, the coordinates of this box are changed and this box is returned, otherwise a new box is
     *            created.
     * @param x1 x-coordinate of corner number 1.
     * @param y1 x-coordinate of corner number 1.
     * @param x2 x-coordinate of corner number 2, which is opposite corner 1.
    ...
    /**
     * Find an element in the supplied subgraph that contains the given point. The last element encoutered is returned
     * from a search of first subgraphs, then edges, then nodes. The ordering within a set of elements (e.g., nodes) is
     * indeterminate.
     *
     ...
     * @return a vector whose components may be single elements or another vector of this same type with the property
     *         that all the elements in the vector (when eventually unravelled) are contained in the supplied box.
     */
    ...
    /**
     * Set the highlight on an element and, possibly, related elements. Since deletion can affect related elements
     * (i.e., the edges connected to a node or the sub-elements of a subgraph), those elements are affected as well when
     * highlighting.
     *
     * @param elem the element whose highlighting is to be adjusted.
     * @param mode the highlight mode to apply or remove; a mode of zero indicates all highlighting is turned off
    ...
    /**
     * Filter the supplied graph using the given connector. The connector is either a java.lang.Process or a
     * java.net.URLConnection. As such, it provides an output stream to which the graph can be written and an input
     * stream from which the processed graph can be read back in (to replace the original graph). Such filtering is
     * useful for processing the graph through a layout engine such as the <i>dot</i> program.
     * <P>
     * Unlike previous versions of Grappa, this version does not try to explicitly redraw the graph after filtering is
     * completed.
     *
     * @param graph the graph to be processed and reset
     * @param connector a Process or URLConnector that provides an input and output stream
     ...
     * java.net.URLConnection. As such, it provides an output stream to which the graph can be written and an input
     * stream from which the processed graph can be read back in (to replace the original graph). Such filtering is
     * useful for processing the graph through a layout engine such as the <i>dot</i> program. The existing graph is
     * reset and its contents are replaced with the graph that is read in.
     * <P>
     * Unlike previous versions of Grappa, this version does not try to explicitly redraw the graph after filtering is
     * completed.
     *
    ...
    public static boolean filterGraph(Graph graph, Object connector, String preamble)
    {
        ...
        try {
            ...
            while ((line = fromFilter.readLine()) != null) {
                ...
                /*
                 * Need to append new-line on the chance that there was a backslash-newline (otherwise need to test for
                 * a lone backslash at the end of the string and remove it... cheaper to just append a newline.
                 */
                newGraph.append(GrappaConstants.NEW_LINE);
            }
        } catch (Exception ex) {
            Grappa.displayException(ex);
            status = false;
    ...
    /**
     * Scroll to the viewport containing the specified GrappaPanel so that it is centered on the given point. If the
     * point is not contained in the subgraph being displayed in the GrappaPanel, no action is taken. If the getParent()
     * method applied to the GrappaPanel argument does not return a JViewport, an error message is displayed.
     *
    ...
    public static boolean centerPanel(Point2D cpt, GrappaPanel gpanel)
    {
        ...
        Rectangle2D r =
            new Rectangle2D.Double(p.getX() + b.x - (d.width) / 2., p.getY() + b.y
