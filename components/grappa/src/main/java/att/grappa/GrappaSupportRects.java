 ...
 *
 * @version $Id: 3dfdd5ee5b443ce35f4ef4223f365dd99e0d75dc $
...
public class GrappaSupportRects implements GrappaConstants
{
    ...
    protected static synchronized Object[] parseRecordInfo(Node node)
    {
        ...
        tableField.positionFields(new Point((int) Math.round(pos.getX() - width / 2.0), (int) Math.round(pos.getY()
            - height / 2.0)));
        ...
        if (emitFields(tableField, objs)) {
            ...
            node.setAttribute(RECTS_ATTR, (objs[2]));
        } else {
            objs = null;
        }
        rbuf = null;
...
class TableField implements GrappaConstants
{
    ...
    boolean hasFields()
    {
        if (this.subFields == null || this.subFields.length == 0 || this.subFieldsUsed == 0) {
            return false;
        }
        return true;
    }
    ...
    void setId(String id)
    {
        ...
        if (hadNBSP) {
            this.idTag = new String(array, 0, array.length);
        } else {
            this.idTag = id;
        }
    }
    ...
    void resizeUpFields(TableField tf, Dimension sz)
    {
        ...
        for (int cnt = 0; cnt < fc; cnt++) {
            ...
            amt = (int) Math.floor((cnt + 1) * incr) - (int) Math.floor((cnt) * incr);
            if (tf.isLR()) {
    ...
    void setTextBounds(String str, Node node)
    {
        ...
        this.textBounds = new Rectangle(0, 0, width, height * lines);
        this.text = str;
    }
