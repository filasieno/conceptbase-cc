 ...
 * </UL>
 * Grappa itself does not have any methods for graph positioning. Grappa simply draws the nodes and edges based on the
 * value of the element's <TT>pos</TT> attribute, though it will treat an unpositioned edge as a straight line between
 * the center points of its attached nodes. Some graph layout references are:
 * <OL>
 * <LI>"An Algorithm for Drawing General Undirected Graphs", Tomihisa Kamada and Satoru Kawai, <I>Information Processing
 * Letters</I>, Vol. 31 (1989), pp. 7 - 15
 * <LI>"Graph Drawing by Force-directed Placement", Thomas M. J. Fruchterman and Edward M. Reingold, <I>Software -
 * Practice and Experience</I>, Vol. 21 No. 11 (1991), pp. 1129 - 1164
 * <LI>"A Technique for Drawing Directed Graphs", Emden R. Gansner, Eleftherios Koutsofios, Stephen C. North and
 * Kiem-Phong Vo, <I>IEEE Transactions on Software Engineering</I>, Vol 19 No. 3 (1993), pp. 214 - 230
 * <LI>"NicheWorks - Interactive Visualization of Very Large Graphs", Graham J. Wills, <A
 * HREF="http://www.bell-labs.com:80/user/gwills/NICHEguide/nichepaper.html"
 * TARGET="_blank">http://www.bell-labs.com:80/user/gwills/NICHEguide/nichepaper.html</A>, circa 1997
 * </OL>
 * <P>
 ...
 *
 * @version $Id: 84010552a9c7f9bb76427fc0e0b8830e4b77d95d $
...
public abstract class Grappa implements GrappaConstants
{
    ...
    /**
     * The java.awt.Toolkit.getDefaultToolkit() value, if available. It is not populated until required, so if not graph
     * display is requested, it will remain null.
     */
    ...
    /**
     * Boolean to indicate if all element attributes should be printed. By default, only the element specific attributes
     * are printed and attributes inherited from a parent element (subgraph) are skipped.
     *
    ...
    /**
     * Boolean to indicate if the default attributes associated with a subgraph should be printed. By default, only
     * those differing from the pre-defined defaults are printed.
     *
     * @see Subgraph#printSubgraph
    ...
    /**
     * Indicates if element node position indicates the node center point. Otherwise, the position is assumed to
     * indicate the upper-left corner of the nodes bounding box. By default, the position indicates the node's center
     * point.
     */
    public static boolean centerPointNodes = true;

    /**
     * Indicates if the label position for node labels should automatically be set to the center point of the node. When
     * true, positioning information provided by the <TT>lp</TT> attribute is completely ignored. By default,
    ...
    /**
     * Indicates if the <TT>bb</TT> attribute of a subgraph should automatically be set whenever the bounding box is
     * calculated. By default, the attribute is not set automatically.
     */
    public static boolean provideBBoxAttribute = false;

    /**
     * Indicates what winding rule to use whenever a winding rule is required. The default is the
     * java.awt.geom.PathIterator WIND_NON_ZERO rule.
     */
    public static int windingRule = java.awt.geom.PathIterator.WIND_NON_ZERO;

    /**
     * Indicates whether the <TT>orientation</TT> attribute is specifed in degrees rather than radians. Degrees is the
     * default.
     */
    public static boolean orientationInDegrees = true;

    /**
     * Indicates whether the <TT>rotation</TT> attribute is specifed in degrees rather than radians. Degrees is the
     * default.
     */
    public static boolean rotationInDegrees = true;

    /**
     * Indicates whether only the list of attributes found in the PRINTLIST_ATTR should be printed. See also
     * printVisibleOnly. The default is false.
     */
    ...
    /**
     * Indicates whether the value of y-coordinates should be negated when reading or writing y-coord information as
     * string attributes. Note: this indicator should be set to <I>true</I> when working with string attributes
     * generated by or to be read by the <TT>dot</TT> graph layout program or to be compatible to earlier versions of
     * Grappa. The default is true.
     */
    public static boolean negateStringYCoord = true;

    /**
     * Indicates that graph labels, when not explicitly positioned via the <TT>lp</TT> attribute, should be placed at
     * the bottom of the graph instead of the top. The default is true, meaning the label will be placed at the bottom
     * of the graph.
     */
    public static boolean labelGraphBottom = true;

    /**
     * Indicates that graph labels, when not explicitly positioned via the <TT>lp</TT> attribute, should be placed just
     * outside the graph bounding box instead of just inside. The default is true, meaning the label will be placed
     * outside the bounding box.
     */
    public static boolean labelGraphOutside = true;

    /**
     * Indicates that background drawing, if any is provided via a <TT>GrappaBacker</TT> implementation, should be
     * displayed or not. The default of true means the background drawing should be displayed, if provided.
     */
    public static boolean backgroundDrawing = true;

    /**
     * When the transform scale applied when drawing in a GrappaPanel is less than this value, then node labels are
     * suppressed.
     */
    public static double nodeLabelsScaleCutoff = 0.5;

    /**
     * Cluster subgraphs will have their bounding box outlined. To similarly outline all types of subgraphs (except the
     * root subgraph), set this value to true.
     */
    public static boolean outlineSubgraphs = false;

    /**
     * When the transform scale applied when drawing in a GrappaPanel is less than this value, then edge labels are
     * suppressed.
     */
    public static double edgeLabelsScaleCutoff = 0.5;

    /**
     * When the transform scale applied when drawing in a GrappaPanel is less than this value, then subgraph labels are
     * suppressed.
     */
    public static double subgLabelsScaleCutoff = 0.3;

    /**
     * Indicates whether paints should be done within a synchronized wrapper. When enable the Graph dropcloth method can
     * be used to prevent paints during certain critical operations.
     *
    ...
    /**
     * Indicates which classes of elements are suitable for selection based on cursor position. Value is the logical OR
     * of NODE, EDGE and SUBGRAPH. The default is SUBGRAPH|NODE|EDGE.
     */
