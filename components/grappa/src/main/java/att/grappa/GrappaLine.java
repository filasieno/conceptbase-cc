 ...
 *
 * @version $Id: 3966ce540a40721d7219424fd6d0de18de68b116 $
...
public class GrappaLine implements GrappaConstants, Cloneable, Shape
{
    ...
    /**
     * Bit flag to indicate that line has arrow heads at both ends. Note that NONE_ARROW_EDGE + HEAD_ARROW_EDGE +
    ...
    /**
     * Constructs a new <code>GrappaLine</code> object from an array of (cubic) curve points. The winding rule for this
     * path is defaulted (from Grappa.windingRule).
     *
     * @param pts the <code>GrappaPoint</code> array used to describe the curve
     * @param type indicates arrow type (NONE_ARROW_EDGE,HEAD_ARROW_EDGE, TAIL_ARROW_EDGE,BOTH_ARROW_EDGE)
     */
    ...
    /**
     * Constructs a new <code>GrappaLine</code> object from a string of (cubic) curve points as used by "dot". All of
     * the initial geometry and the winding rule for this path are defaulted.
     *
     * @param curve the <code>String</code> that specifies the point list; the format is: [s,x0,y0|e,xN,yN] [x1,y2] ...
     *            [xN-1,yN-1]
     */
    ...
    public boolean equals(Object obj)
    {
        ...
        if (cmp.gpts.length != this.gpts.length || !this.gpts.equals(cmp.gpts)) {
            return (false);
        }
     ...
     *
     * @return true if the line is oriented so that its starting point is nearer to the supplied point than its ending
     *         point.
     */
     ...
     *
     * @param pointFormat the specific format directive to use for each point in the line (<TT>%p</TT> is the base
     *            directive).
    ...
    public String toFormattedString(String pointFormat)
    {
        ...
        if ((this.arrow & TAIL_ARROW_EDGE) != 0) {
            if (spacer) {
                buf.append(" e,");
            } else {
                buf.append("e,");
     ...
     *
     * @param new_type indicates arrow type (NONE_ARROW_EDGE,HEAD_ARROW_EDGE, TAIL_ARROW_EDGE,BOTH_ARROW_EDGE)
     * @return true if the type changed, false otherwise.
     */
    public boolean changeArrowType(int new_type)
    {
        boolean changed = false;
        if (this.arrow != new_type && (new_type & (~(BOTH_ARROW_EDGE))) == 0) {
            changed = true;
            updateLine(this.gpts, new_type);
        }
        return (changed);
    }
    ...
    private void addArrow(GeneralPath path, GeneralPath testpath, GrappaPoint tip, GrappaPoint shaft, double length,
        double width)
    {
