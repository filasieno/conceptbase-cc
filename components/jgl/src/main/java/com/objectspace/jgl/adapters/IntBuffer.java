 ...
 * IntArray, except that it can expand to accomodate new elements.
 * <p>
 ...
 * @see com.objectspace.jgl.adapters.IntIterator
 * @version 3.1.0
...
public class IntBuffer implements Sequence
  ...
  public synchronized void clear()
    {
    storage = new int[ ArrayAdapter.DEFAULT_SIZE ];
    length = 0;
    }
  ...
  public int remove( Enumeration first, Enumeration last )
    {
    if ( !( first instanceof IntIterator && last instanceof IntIterator ) )
      throw new IllegalArgumentException( "Enumeration not an IntIterator" );

    if ( ( (IntIterator)first ).buffer != storage || ( (IntIterator)first ).isCompatibleWith( (IntIterator)last ) )
      throw new IllegalArgumentException( "Enumeration not compatible" );

    return remove( ( (IntIterator)first ).index, ( (IntIterator)last ).index - 1 );
    }
   ...
   * @exception com.objectspace.jgl.InvalidOperationException If the IntBuffer is empty.
   */
   ...
   * @param object The int for which ot search.
   */
