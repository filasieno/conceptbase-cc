...
/**
 * A ShortBuffer is a sequence that is very similar to a regular 
 * ShortArray, except that it can expand to accomodate new elements.
 * <p>
 * The implementation store elements in a contiguous linear memory space 
 * so that index-based access is very quick. When an ShortBuffer's originally 
 ...
 * @see com.objectspace.jgl.adapters.ShortIterator
 * @version 3.1.0
...
public class ShortBuffer implements Sequence
  ...
  public ShortBuffer( int size )
    ...
    if ( size < 0 )
      throw new IllegalArgumentException
        ( 
        "Attempt to create an ShortBuffer with a negative size" 
        );
   ...
   * another ShortBuffer.
   * @param array The ShortBuffer to compare myself against.
   */
  public boolean equals( ShortBuffer buffer )
    {
    return equals( new ShortArray( buffer.storage ) );
    }
   ...
   */
  public synchronized boolean equals( ShortArray array )
    {
  ...
  public boolean equals( short array[] )
    {
    return equals( new ShortArray( array ) );
    }
   ...
   * @param array The ShortBuffer that I shall become a shallow copy of.
   */
  ...
  public synchronized void clear()
    {
    storage = new short[ ArrayAdapter.DEFAULT_SIZE ];
    length = 0;
    }
  ...
  public int remove( Enumeration first, Enumeration last )
    {
    if ( !( first instanceof ShortIterator && last instanceof ShortIterator ) )
      throw new IllegalArgumentException( "Enumeration not an ShortIterator" );

    if ( ( (ShortIterator)first ).buffer != storage || ( (ShortIterator)first ).isCompatibleWith( (ShortIterator)last ) )
      throw new IllegalArgumentException( "Enumeration not compatible" );

    return remove( ( (ShortIterator)first ).index, ( (ShortIterator)last ).index - 1 );
    }
   ...
   * @exception com.objectspace.jgl.InvalidOperationException If the ShortBuffer is empty.
   */
  ...
  /**
   * Add a short after my last element.  Returns null.
   * This function is a synonym for pushBack().
   * @param object The object to add.
   */
  ...
  public ShortIterator insert( ShortIterator pos, short object )
    ...
    return new ShortIterator( this, pos.index );
    }
   ...
   * @param array The ShortBuffer that I will swap my contents with.
   */
  ...
  /**
   * Replace all shorts that match a particular object with a new value and return
   * the number of objects that were replaced.
   * @param oldValue The short to be replaced.
   * @param newValue The value to substitute.
   */
  ...
  /**
   * Replace all values within a range of indices that match a particular short
   * with a new value and return the number of objects that were replaced.
   ...
   * @param object The short to count.
   */
  ...
  /**
   * Return the index of the first short that matches a particular value, 
   * or -1 if the value is not found.
   * @param object The short to find.
   */
  ...
  /**
   * Return the index of the first short within a range of indices that 
   * match a particular value, or -1 if the short is not found.
  ...
  /**
   * Sets the size of the ShortBuffer. if the size shrinks, the extra elements (at
   * the end of the array) are lost; if the size increases, the new elements
   * are set to null.
   * @param newSize The new size of the ShortBuffer.
   */
   ...
   * @param object The short for which ot search.
   */
