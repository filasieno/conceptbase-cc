...
/**
 * A FloatBuffer is a sequence that is very similar to a regular 
 * FloatArray, except that it can expand to accomodate new elements.
 * <p>
 * The implementation store elements in a contiguous linear memory space 
 * so that index-based access is very quick. When an FloatBuffer's originally 
 ...
 * @see com.objectspace.jgl.adapters.FloatIterator
 * @version 3.1.0
 * @author ObjectSpace, Inc.
 */

public class FloatBuffer implements Sequence
  {
  final static float defaultValue = 0.0f;
  float storage[]; // My storage.
  ...
  public FloatBuffer( int size )
    ...
    if ( size < 0 )
      throw new IllegalArgumentException
        ( 
        "Attempt to create an FloatBuffer with a negative size" 
        );
   ...
   * @exception java.lang.IllegalArgumentException If size is negative.
   */
  public FloatBuffer( int size, float object )
    {
    this( size );
  ...
  FloatBuffer( float array[], boolean copyBuffer )
    {
    synchronized( array )
      {
      length = array.length;
   ...
   */
  public FloatBuffer( FloatBuffer array )
    {
    this( array.storage );
    }
   ...
   * another FloatBuffer.
   * @param array The FloatBuffer to compare myself against.
   */
  public boolean equals( FloatBuffer buffer )
    {
    return equals( new FloatArray( buffer.storage ) );
    }
   ...
   */
  public synchronized boolean equals( FloatArray array )
    {
  ...
  public boolean equals( float array[] )
    {
    return equals( new FloatArray( array ) );
    }
   ...
   * @param array The FloatBuffer that I shall become a shallow copy of.
   */
  public synchronized void copy( FloatBuffer buffer )
    {
    if ( this == buffer )
      return;
   ...
   */
  public synchronized void copyTo( float[] array )
    {
    synchronized( array )
      {
  ...
  public synchronized void clear()
    {
    storage = new float[ ArrayAdapter.DEFAULT_SIZE ];
    length = 0;
    }
  ...
  public synchronized Object remove( int index )
    {
    ArrayAdapter.checkIndex( index, length );
    Object retval = new Float( storage[ index ] );
  ...
  public int remove( Enumeration first, Enumeration last )
    {
    if ( !( first instanceof FloatIterator && last instanceof FloatIterator ) )
      throw new IllegalArgumentException( "Enumeration not an FloatIterator" );

    if ( ( (FloatIterator)first ).buffer != storage || ( (FloatIterator)first ).isCompatibleWith( (FloatIterator)last ) )
      throw new IllegalArgumentException( "Enumeration not compatible" );

    return remove( ( (FloatIterator)first ).index, ( (FloatIterator)last ).index - 1 );
    }
   ...
   * @exception com.objectspace.jgl.InvalidOperationException If the FloatBuffer is empty.
   */
  public synchronized Object popBack()
    ...
    Object r = new Float( storage[ --length ] );
    storage[ length ] = defaultValue;
    return r;
    }
  ...
  /**
   * Add a float after my last element.  Returns null.
   * This function is a synonym for pushBack().
   * @param object The object to add.
   */
  public synchronized void add( float object )
    {
    if ( length == storage.length )
      {
      float[] tmp = getNextStorage( 1 );
      copyTo( tmp );
      storage = tmp;
      }
   ...
   * @param The object to add.
   */
  public void pushBack( float object )
    {
    add( object );
    }
   ...
   */
  public FloatIterator insert( FloatIterator pos, Object object )
    {
    return insert( pos, asFloat( object ) );
    }
   ...
   * @param object The object to insert.
   */
  public FloatIterator insert( FloatIterator pos, float object )
    {
    insert( pos.index, object );
    return new FloatIterator( this, pos.index );
    }
   ...
   * @exception java.lang.IndexOutOfBoundsException If the index is invalid.
   */
  public synchronized void insert( int index, float object )
    {
   ...
   * @param object The object to insert.
   */
  public void insert( FloatIterator pos, int n, Object object )
    {
    insert( pos, n, asFloat( object ) );
    }
   ...
   * @param object The value to insert.
   */
  public void insert( FloatIterator pos, int n, float object )
    {
   ...
   *   is negative.
   */
  public synchronized void insert( int index, int n, float object )
    {
    if ( n < 0 )
   ...
   */
  public void insert( FloatIterator pos, ForwardIterator first, ForwardIterator last )
    {
   ...
   * @param array The FloatBuffer that I will swap my contents with.
   */
  public synchronized void swap( FloatBuffer array )
    {
    synchronized( array )
      {
  ...
  public synchronized FloatIterator end()
    {
    return new FloatIterator( this, length );
    }
  ...
  public synchronized Object popFront()
    ...
    Object result = new Float( storage[ 0 ] );
    remove( 0 );
    return result;
    }
   ...
   * @param object The object to insert.
   */
  public void pushFront( float object )
    {
    insert( 0, object );
    }
  ...
  public synchronized int remove( int first, int last, Object object )
    {
    asFloat( object ); // check param
    if ( last < first )
      return 0;

    ArrayAdapter.checkRange( first, last, length );
    FloatIterator firstx = new FloatIterator( this, first );
    FloatIterator lastx = new FloatIterator( this, last + 1 );
    FloatIterator finish = (FloatIterator)Algorithms.Removing.remove( firstx, lastx, object );
  ...
  public int replace( Object oldValue, Object newValue )
    {
    return replace( asFloat( oldValue ), asFloat( newValue ) );
    }

  /**
   * Replace all floats that match a particular object with a new value and return
   * the number of objects that were replaced.
   * @param oldValue The float to be replaced.
   * @param newValue The value to substitute.
   */
  ...
  public int replace( int first, int last, Object oldValue, Object newValue )
    {
    return replace( first, last, asFloat( oldValue ), asFloat( newValue ) );
    }

  /**
   * Replace all values within a range of indices that match a particular float
   * with a new value and return the number of objects that were replaced.
   ...
   * @exception java.lang.IndexOutOfBoundsException If either index is invalid.
   */
  public synchronized int replace( int first, int last, float oldValue, float newValue )
    {
    ArrayAdapter.checkRange( first, last, length );
    int n = 0;
    while ( first < last )
  ...
  public int count( Object object )
    {
    return count( asFloat( object ) );
    }

  /**
   * Return the number of objects that equal a particular value.
   * @param object The float to count.
   */
  public int count( float object )
    {
  ...
  public int count( int first, int last, Object object )
    {
    return count( first, last, asFloat( object ) );
    }
   ...
   *   invalid.
   */
  public synchronized int count( int first, int last, float object )
    {
    ArrayAdapter.checkRange( first, last, length );
    int n = 0;
    while ( first < last )
  ...
  public int indexOf( Object object )
    {
    return indexOf( asFloat( object ) );
    }

  /**
   * Return the index of the first float that matches a particular value, 
   * or -1 if the value is not found.
   * @param object The float to find.
   */
  public int indexOf( float object )
    {
  ...
  public int indexOf( int first, int last, Object object )
    {
    return indexOf( first, last, asFloat( object ) );
    }

  /**
   * Return the index of the first float within a range of indices that 
   * match a particular value, or -1 if the float is not found.
   ...
   *   invalid.
   */
  public synchronized int indexOf( int first, int last, float object )
    {
    if ( last < first )
      return -1;
    ArrayAdapter.checkRange( first, last, length );
  ...
  /**
   * Sets the size of the FloatBuffer. if the size shrinks, the extra elements (at
   * the end of the array) are lost; if the size increases, the new elements
   * are set to null.
   * @param newSize The new size of the FloatBuffer.
   */
  ...
  public boolean contains( Object object )
    {
    return contains( asFloat( object ) );
    }

  /**
   * Return true if I contain a particular float.
   * @param object The float for which ot search.
   */
