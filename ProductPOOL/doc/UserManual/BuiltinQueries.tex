...
\item[concatl:] concatenates the two labels (being the names of any object); the result is 
  stored as an individual with the concatenated labels; the result is shown as a label, not as
  string in quotes
\item[concatl4:] concatenates the four labels (being the names of any object); the result is 
  stored as an individual with the concatenated labels
\item[concatl6:] concatenates the six labels (being the names of any object); the result is 
  stored as an individual with the concatenated labels
...
but yield object names that are not parsable by ConceptBase when used in a frame. Use {\tt toLabel}
if you want to be sure that the label is a correct object name according to the syntax of Telos
frames.
