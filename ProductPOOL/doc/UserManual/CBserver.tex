...
\item[-team] display the ConceptBase developers who contributed to the current source code and exit.
...
this user can always connect to the CBserver.
...
\subsection{Updating the CBserver software}

You can always update your local installation of ConceptBase downloading and executing 
the interactive CBinstaller.jar Java installer from \url{https://conceptbase.sourceforge.net}.
Be sure to terminate the ConceptBase server and the ConceptBase user interface programs
before updating the software.

An alternative to the interactive update via CBinstaller.jar is the shell script

\begin{verbatim}
   updateCB-bin
\end{verbatim}

located in the ConceptBase installation directory. We recommend to update the ConceptBase software
at least once per year.


\section{ConceptBase under Windows 10/11}
\label{sec:win10}
...
Since April 2017, this detour is no longer required for users of Windows 10/11 (64bit).
This version of Windows is capable to let Linux programs
...
natively on Windows 10/11.

To enable the Linux capability on Windows 10/11, follow the instructions at
...
The string "64bit" should be in the response. Users of older Windows versions and of other operating systems can continue to use the
...
It can be set by the menu item "Options/Edit Options" of CBIva, see section \ref{sec:config}.
...
\footnote{Windows 10/11 (64bit) has the ability to activate a Linux sub-system
which can then be used to start CBserver transparently.
If you had previously used a public CBserver under Windows 10/11 and now want to utilize a local CBserver,
...
("localhost"). The same is true for Windows 10/11 with an enabled Linux subsystem. Users on Mac
...
limited to Linux and Windows 10/11 (see also section \ref{sec:win10}). In other cases,
...
\item[-un username] configures CBShell to use "username" instead the username as assigned by the 
operating system to connect to a ConceptBase server. Only required in special circumstances, e.g.
when your own username shall not be disclosed.
...
\item[lpicall {\it lpicall}] :
      executes the LPI call; only for debugging purposes; disabled for security reasons

\item[prolog {\it prologstatement}] :
      executes the Prolog statement; only for debugging purposes; disabled for security reasons
...
\item[cd {\it mod}] :
      ...
      the command "cd .." shall switch to the super module of the currenbt module, the command
      "cd ." shall leave the current module unchanged
