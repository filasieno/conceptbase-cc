...
represenation of an object by defining graphical attributes such as
...
\item[bgimage:] sets the background image for the graph windows; the image shall
  be specified by the URL to a PNG, GIF, or JPG image; it is scaled by the 
  CBGraph editor to fit into the internal window showing a graph of this palette
...
Implicit In & ImplicitInstanceOfGT & dashed green line\\ \hline
Implicit IsA & ImplicitIsAGT & dashed blue line; white edge heads\\ \hline
Implicit Attribute & ImplicitAttributeGT & dashed black line \\ \hline
\end{tabular}
\end{center}
...
The safest way to define a correct graphical palette is to use
the intermediate {\tt XPalette} as superclass of the new palette.
It can be found within the CB-Forum model
for business model canvasses at
\url{http://merkur.informatik.rwth-aachen.de/pub/bscw.cgi/3594885}.
With {\tt XPalette} you can define a new
palette without having to worry about values for required
...
\begin{itemize}
\item Defining new graphical types with different
properties using the provided implementations\\ i5.cb.graph.cbeditor.CBIndividual
and i5.cb.graph.cbeditor.CBLink
\item Defining new graphical types with a different
implementation class which extends\\ i5.cb.graph.cbeditor.CBUserObject
(or CBIndividual or CBLink)
\end{itemize}

Both possibilities will presented in the next two
subsections.

\subsection{Properties of CBIndividual and CBLink}
...
\item[size:]
    ...
    the non-numeric value "resizable" is allowed as well.
    If set, the user can also resize the element via mouse actions.
    (default: undefined, then the size is fixed by the Java LayoutManager)
...
is called using the object name as single parameters. For example, if {\tt t1} is the
...
Linux/Unix. Adapting the source code requires considerable programming skills in Java.
...
An example for the use of resizable shapes graphical types can be found in
the CB-Forum at
...
provided that the access rights are set properly.
...
(starting with "http://" or "file://") or a relative file location such as
"diaicons/icon1.png". In the latter case, CBGraph shall first check if local
