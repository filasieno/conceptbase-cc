...
window should pop up. Figure \ref{fig:cbivamain} shows the main window connected
to a server and gives a short description of the buttons in the tool bar.


\cbfigure{cbivamain}{13cm}{Main Window of CBIva}
...
\item[Browse:]
	...
	\item[Display Queries:] Shows all queries stored in the current database and provides a facility to
...
\begin{itemize}
\item Quick Connect: if connected to a CBserver then disconnect; otherwise try to
...
\item Clear
\item Cut, Copy, Paste
\item Tell, Untell: The objects specified in the active Telos Editor will be added or removed
from the object base.
\item Retell: A new window will popup and ask you to enter the frames that should be untold and
told. The contents of the current Telos Editor will be inserted as default into the two text areas.

\item Ask: Evaluate the query specified in the Telos Editor. \footnote{The query can either be a query call
...
\item List module: List the content of the current module as Telos frames
\item Exit
\end{itemize}
...
\begin{itemize}
\item Connection status, either connected or disconnected from server
\item Short message, usually the result of the last action
...
\item[Display in Graph Editor:] Shows the object in the current window of CBGraph. CBGraph
 has to be started before (menu Browse)
...
connection details of the CBserver module from which the graph was created.
...
\item[+r] With this option CBGraph will be instructed to load the module sources on its current module
  ...
  CBserver when CBGraph is started.
...
\item[-host hostport] Instruct CBGraph to use the hostname and portnumber from hostport rather than those
   defined in the graph file when loading the graph file.
\end{description}
...
\subsection{Connecting CBIva to the CBserver}

Next we establish a connection between the {\em ConceptBas.cc}  server and the user
