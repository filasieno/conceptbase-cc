...
\item[{\tt LABEL}:]
      ...
      {\tt bill, mary, john}
\item[{\tt FRAME}:]
      the answer shall be returned as a list of Telos frames, iff the query
      does not have a user-defined answer format, see {\tt forQuery} attribute
      below. If there is an answer format with a {\tt forQuery} attribute
      for the current query, it will be selected. If there are more than one,
      the first one is selected.
\item[{\tt default}:]
      This value is leaving the choice to the ConceptBase server. For function
      calls, the answer format {\tt LABEL} is selected, for other query calls, the format
      {\tt FRAME} is selected.
      The main purpose of this format is to provide a more intuitive behavior of CBIva
      when used for interactively querying ConceptBase. 
...
By default, ConceptBase displays answers to queries in the FRAME format (see `A' and `B' below). For many applications, other answer
...
\item \verb+{QT(expr)}+ puts the {\tt expr} into double quotes. 

\item \verb+{UQ(expr)}+ removes double quotes from {\tt expr}.
...
\verb+{ASKquery(Q[subst1,subst2,...],FRAME)}+
...
`FRAME' is the default answer format specified for the query Q. It shall be overridden
in case that query Q has itself an answer format. Instead of `FRAME' the user can also
specify any answer format in the ASKquery call, even parameterized answer formats (see below)
are allowed.

The effect of ASKquery in an answer format is that the above query call is evaluated
and the ASKquery expression is replaced by the complete answer to the query.
In terms of the substitution, the following rule is applied:

   \verb+{ASKquery(Q[subst1,subst2,...],FRAME)}+ $\longrightarrow$ {\tt X}
...
\newline
\verb+{ASKquery(MyQuery[{this.cat1}/param1,{this.{x1}}/{x2}],FRAME)}+.
\newline
...
\begin{tabular}{rll}
   & \verb+{ASKquery(MyQuery[{this.cat1}/param1,{this.{x1}}/{x2}],FRAME)}+ & \\
 $\Longrightarrow$ & \verb+{ASKquery(MyQuery[alpha/param1,{this.{x1}}/{x2}],FRAME)}+ & (r2)\\
 $\Longrightarrow$ & \verb+{ASKquery(MyQuery[alpha/param1,{this.name}/{x2}],FRAME)}+ & (r1)\\
 $\Longrightarrow$ & \verb+{ASKquery(MyQuery[alpha/param1,"smith"/{x2}],FRAME)}+     & (r3)\\
 $\Longrightarrow$ & \verb+{ASKquery(MyQuery[alpha/param1,"smith"/param2],FRAME)}+   & (r4)\\
...
r5:  & \verb+{ASKquery(MyQuery[...],FRAME)}+ & $\longrightarrow$ & {\tt The answer is ...}\\
\end{tabular}

This guarantees that the query call is `ground', i.e. does not contains terms which are subject to substitution.

The ASKquery construct allows to introduce recursive calls during the derivation of a query since there can (and should) be an answer format for Q
which may contain expressions ASKquery itself. In principle, this allows infinite looping. However, the answer format evaluator prevents such
...
A simple example for use of ASKquery is given in {\tt recursive-answers.sml}. The example uses a view instead of a query class in order to include
...
It is common practice to combine the ASKquery construct with `Foreach' in order to display an iteration of objects in the same way. The user
should define an answer format for the iterated query Q as well.

Do not mix the use of ASKquery with the view definitions in ConceptBase! The nesting depth of a view is determined by the view definition. The
nesting depth of an answer generated by expansion of ASKquery is only limited by the complexity of the database. For example, one can set up
...
The nested ASKquery inside an answer format usually results in the unfolding also using
...
This only leaves function patterns such as ASKquery expressions and pre-defined patterns such as  \verb+{user}+.
...
There are three predefined answer representations. {\tt FRAME} returns the answers as
Telos frames, including retrieved and computed attributes. {\tt LABEL} returns only
the names of the answer objects as a comma-separated list. Thirdly, {\tt default}
lets the CBserver choose between {\tt LABEL}  (for function calls) and {\tt FRAME}
(otherwise). Besides these pre-defined answer representations, one can specify
...
The main prupose of bulk queries is to speed up the interaction between the ConceptBase clients,
