...
default tracemode is set to {\tt minimal} (only display CBserver interface).
...
The -r option is not available under Windows.
...
permission rules as defined in section \ref{sec:modaccess}. Enable access control
...
cases where metaformulas are defined and used in the same module.
...
exist, shall use a default public CBserver. This is the case for Windows and OS-X.

Don't forget to save the options after changing the variable.
Since the Windows/OS-X variants of the CBserver are no longer supported, the CBGraph tool
must use a public CBserver when started on Windows and OS-X.
...
to 20000. Exceeding the maximum is not an error. It only indicates that the cache
...
results. The 'keep' mode is the default mode for tabling.
...
The API to the ConceptBase server (CBserver) is realized by the Java class 
{\tt LocalCBclient}.
...
name of the computer on which your CBserver runs on) and {\tt iPort} (the port number as 
...
The return value is {\tt "yes"} if successful and {\tt "no"} else.
\end{itemize}
...
The Java source code {\tt TinyClient.java} and for a more elaborate example 
{\tt SimpleClient.java} are included in the directory {\tt examples/Clients/JavaClient}
...
\item[ask {\it Query} {[{\it QueryFormat} [{\it AnswerRep} [{\it RollbackTime}]]]}] :
      ...
      be {\tt LABEL}, {\tt FRAME}, {\tt default}, or a user-defined answer format; the rollback
      time should be
      {\tt Now}. If the query format is {\tt OBJNAMES}, then {\it Query} is a string
